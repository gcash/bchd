...
// It is the signed response to the avaquery message.
// TODO: Change response format to be a list of rejected vertex hash prefixes
// and a series of binary votes for accepted/no-vote. This begins saving a bit
// per vote after $prefix_length votes.
...
// the query ID that is suitable for signing.
func (msg *MsgAvaResponse) SerializeForSignature() []byte {
	var buf bytes.Buffer
	reqIDBytes := make([]byte, 8)
	binary.LittleEndian.PutUint64(reqIDBytes, msg.QueryID)
...
func NewMsgAvaResponse(queryID uint64, votes []byte, signature *bchec.Signature) *MsgAvaResponse {
