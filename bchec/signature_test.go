...
func TestRFC6979(t *testing.T) {
	...
	for i, test := range tests {
		...
		gotSig, err := privKey.SignECDSA(hash[:])
		if err != nil {
...
func TestSchnorrSignatureVerify(t *testing.T) {
	...
	tests := []struct {
		...
		{
			decodeHex("C90FDAA22168C234C4C6628B80DC1CD129024E088A67CC74020BBEA63B14E5C7"),
	...
	for i, test := range tests {
		...
		if len(test.privKey) != 0 {
			privKey, _ := PrivKeyFromBytes(S256(), test.privKey)
			if !bytes.Equal(privKey.PubKey().SerializeCompressed(), pubkey.SerializeCompressed()) {
				t.Errorf("TestSchnorrSignatureVerify test %d invalid public key", i)
			}
		}
