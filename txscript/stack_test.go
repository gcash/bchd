...
func TestStack(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nil,
			func(s *stack) error {
				s.PushInt(*makeScriptNumFromInt(0))
				return nil
			},
			nil,
			[][]byte{{}},
		},
		{
			...
			nil,
			func(s *stack) error {
				s.PushInt(*makeScriptNumFromInt(1))
				return nil
			},
			nil,
			[][]byte{{0x1}},
		...
		},
		{
			...
			nil,
			func(s *stack) error {
				s.PushInt(*makeScriptNumFromInt(256))
				return nil
			},
			nil,
			// little endian.. *sigh*
		...
		},
		{
			...
			func(s *stack) error {
				...
				s.PushInt(*makeScriptNumFromInt(128))
				return nil
			},
			nil,
			[][]byte{{0x80, 0x00}},
